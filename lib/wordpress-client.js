    ...
    value: function detectBlog(callback) {
      ...
      function detectCallback(error, response) {
        ...
        if (!error) {
          ...
          matchedUrl = matchedUrl.replace("http://", "");
          matchedUrl = matchedUrl.replace("www.", "");
          ...
          if(blog){          
             blogid = parseInt(blog.blogid, 10);
          }
          else{
            //take the first blog found if no match was made
            if(response && response.length>0){
              blogid = response[0].blogid;
              blog = response[0];
            }
          }
